...
class RDTSender:
    ...
    def get_checksum(data):
        ...
        checksum = ord(data)  #converts a character into ascii code
        return checksum
    ...
    def rdt_send(self, process_buffer):
        ...
        for data in process_buffer:
            ...
            while True:
                ...
                pkt = RDTSender.make_pkt(self.sequence, data, checksum)#initialized pkt with seq no 0
                print('sender expecting seq no: '+pkt['sequence_number'])
                print("sender sending: ",pkt)#tracing
                reply = self.net_srv.udt_send(pkt)#send pkt to the transport layer
                print("sender received: ",reply)
                if not RDTSender.is_corrupted(reply) and RDTSender.is_expected_seq(reply, self.sequence):#if its not corrupted and with expected seq no(same seq no as sent, same data as sent (0))
                    if (self.sequence=='0'):
                           self.sequence='1'#toggle
                           break#exit the loop(dont resend) 
                    else:
                           self.sequence='0'
                           break#exit the loop(dont resend) 
                else:
                    if (RDTSender.is_corrupted(reply)):
                        print("corruption occurred:  ",reply)
                    else:
